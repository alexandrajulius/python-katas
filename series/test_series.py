...
def test_series_works_for_49142():
    ...
    actual_series = series(digits, series_length)	
...
def test_series_fails_on_empty_digits():
    assert_that(calling(series).with_args('', 3), raises(ValueError))

def test_series_fails_on_negative_series():
    assert_that(calling(series).with_args('49142', -1), raises(ValueError))

def test_series_fails_on_zero_series():
    assert_that(calling(series).with_args('49142', 0), raises(ValueError))

def test_series_fails_series_higher_than_digit_length():
	assert_that(calling(series).with_args('4', 2), raises(ValueError))

def test_series_fails_if_digit_contains_non_digits():
    assert_that(calling(series).with_args('4a', 1), raises(ValueError))
