...
# It also stores the letters in lower-case regardless of the case of 
# the input letters:
...
# "d" is worth 2 points.
# Etc.
# Your mission, should you choose to accept it, is to transform 
# the legacy data format to the shiny new format.

from typing import Dict, List

def transform(legacy_score:Dict[int, List[str]]) -> Dict[str, int]:
    if not legacy_score:
        raise ValueError()
    shiny_score = dict()  # type: Dict[str, int]
    for score, letters in legacy_score.items():
        for letter in map(str.lower, letters):
            if len(letter) != 1:
                raise ValueError()
            if not letter.isalpha():
                raise ValueError()
            if shiny_score.get(letter, score) != score:
                raise ValueError()
            shiny_score[letter] = score
    return shiny_score